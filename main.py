...
async def lot_selected_delet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    url = f"https://netwbx.ru/api/lot/{query.data}/"
    try:
        response = requests.get(url)
...
async def lot_finish_delete(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    match query.data:
        case 'no_del':
            ...
            return ConversationHandler.END
        case 'yes_del':
            url = f"https://netwbx.ru/api/lot/{context.user_data['lot_id_del']}/"
            headers = {
...
async def lot_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | Any:
    ...
    match query.data:
        case 'add_lot':
            ...
            return NAME
        case 'del_lot':
            url = f"https://netwbx.ru/api/user_lots/{context.user_data['id_tlg']}/"
            try:
                response = requests.get(url)
                response.raise_for_status()
                lots = response.json()
